...
Intra-Transaction: A single transaction cannot reference the same input twice.
Mempool (Race Attack): If an input is already locked by a pending transaction, subsequent attempts to spend it are rejected immediately.
Mining Logic
